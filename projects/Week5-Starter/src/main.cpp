...
std::string windowTitle = "Shesh Gaur - 100786399";
...
int main() {
	...
	camera->SetPosition(glm::vec3(0, 8, 1));
	camera->LookAt(glm::vec3(0.0f));
	camera->SetOrthoVerticalScale(16.0f);
	...
	VertexArrayObject::Sptr vao4 = ObjLoader::LoadFromFile("thePolyTree.obj");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (glfwGetKey(window, GLFW_KEY_SPACE)) {
			if (!isButtonPressed) {
				camera->SetOrthoEnabled(!camera->GetOrthoEnabled());
			}
		...
		if (isRotating) {
			...
			transform2 = glm::translate(glm::mat4(1.0f), glm::vec3(-4.0f, 0.0f, 0.0f));
			transform2 *= glm::rotate(glm::mat4(1.0f), -static_cast<float>(thisFrame), glm::vec3(0, 0, 1));
		}
