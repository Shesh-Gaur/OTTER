...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		transform3 = glm::translate(glm::mat4(1.0f), glm::vec3(glm::cos(static_cast<float>(thisFrame)), 1.0f, -1.0f));
		camera->SetPosition(glm::vec3(0.f, 2.f + glm::sin(static_cast<float>(thisFrame)), 1.f));
		...
		vao3->Bind(); // this should already exist!
		glDrawElements(GL_TRIANGLES, interleaved_ibo->GetElementCount(), (GLenum)interleaved_ibo->GetElementType(), nullptr);
